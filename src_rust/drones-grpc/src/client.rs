...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = Request::new(Registration{name: my_drone.name});
    ...
    while let Some(waypoint) = stream.message().await? {
        ...
        let pos_request = Request::new(drones::Position{latitude: my_drone.lattitude, longitude: my_drone.longitude, altitude: my_drone.altitude});
        client.send_position(pos_request).await?;
